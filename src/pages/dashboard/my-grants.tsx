...
// Backend response interface for my-grants endpoints
interface BackendGrant {
  grant_uid: string;
  title: string;
  agency: string;
...
type GrantCategory = 'liked' | 'disliked' | 'saved';

// GrantCard component - same as grants.tsx
...
const GrantCard: React.FC<GrantCardProps> = ({ grant, reaction, onReact, onGrantClick, category }) => {
  ...
  return (
    ...
    >
      {/* Category Badge */}
      {category && (
        <div className={`absolute -top-2 -right-2 flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium shadow-lg ${
          category === 'liked' 
          ...
          {category === 'liked' ? '❤️ Liked' : category === 'disliked' ? '👎 Disliked' : '🔖 Saved'}
        </div>
      )}
      {/* Match Score Badge (if available) */}
      {grant.matchScore && (
        <div className={`absolute -top-2 -left-2 flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium shadow-lg ${getMatchScoreColor(grant.matchScore)}`}>
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="space-y-2 mb-4">
          ...
          {grant.institution?.name && grant.institution.name !== 'Institution not specified' && (
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <FiHome className="w-4 h-4" />
...
const MyGrantsPage: React.FC = () => {
  useRequireAuth();
  const { user, loading: authLoading } = useAuth();
  const [grants, setGrants] = useState<Grant[]>([]);
  const [loading, setLoading] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState<GrantCategory>('liked');
  ...
  // Fetch grants based on category - using same pattern as grants.tsx
  const loadGrantsByCategory = useCallback(async (category: GrantCategory) => {
    if (!user?.id) return;
    ...
    try {
      ...
      switch (category) {
        case 'liked':
          endpoint = 'liked-grants';
          break;
        case 'disliked':
          endpoint = 'disliked-grants';
          break;
        case 'saved':
          endpoint = 'saved-grants';
          break;
      }
      ...
      let backendGrants: BackendGrant[] = [];
      if (Array.isArray(data)) {
        backendGrants = data;
      } else if (data.grants && Array.isArray(data.grants)) {
        backendGrants = data.grants;
      } else if (data.data && Array.isArray(data.data)) {
        backendGrants = data.data;
      } else if (data.results && Array.isArray(data.results)) {
        backendGrants = data.results;
      }
      
      // Transform backend grants to frontend Grant format
      const grantsData: Grant[] = backendGrants.map(backendGrant => ({
        grant_id: backendGrant.grant_uid,
        title: backendGrant.title,
        abstract: `Grant from ${backendGrant.agency}`, // Fallback since backend doesn't provide abstract
        funding_amount: parseInt(backendGrant.funding_amount) || 0,
        start_date: new Date().toISOString(), // Fallback since backend doesn't provide start_date
        status: 'Active', // Fallback since backend doesn't provide status
        principal_investigators: [], // Fallback since backend doesn't provide PIs
        institution: {
        ...
        research_areas: backendGrant.research_areas,
        agency: backendGrant.agency,
        created_at: new Date().toISOString(),
        source_id: backendGrant.grant_uid,
        matchScore: undefined
      }));
      
      console.log('Transformed grants:', grantsData);
      setGrants(grantsData);
    } catch (error) {
      console.error(`Error fetching ${category} grants:`, error);
      setGrants([]);
    } finally {
      setLoading(false);
    }
  ...
  // Load user reactions for the current grants - same as grants.tsx
  const loadUserReactions = useCallback(async () => {
    ...
    try {
      ...
      // Transform the API response to match our UserReaction type
      const typedReactions: {[grantId: string]: UserReaction} = {};
      Object.entries(reactions).forEach(([grantId, reaction]) => {
        typedReactions[grantId] = {
          reaction_type: (reaction.reaction_type as 'neutral' | 'like' | 'dislike') || 'neutral',
          is_saved: reaction.is_saved || false
        };
      });
      console.log('Fetched reactions from backend:', typedReactions);
  ...
  // Load grants when category changes
  useEffect(() => {
    console.log('useEffect triggered:', { authLoading, userId: user?.id, selectedCategory });
    if (!authLoading && user?.id) {
      console.log('Fetching grants for category:', selectedCategory);
      loadGrantsByCategory(selectedCategory);
    } else {
      console.log('Not fetching grants:', { authLoading, userId: user?.id });
    }
  }, [selectedCategory, user?.id, authLoading, loadGrantsByCategory]);

  // Load user reactions when grants change
  useEffect(() => {
    loadUserReactions();
  }, [loadUserReactions]);

  // Handle category toggle
  const handleCategoryChange = (category: GrantCategory) => {
    setSelectedCategory(category);
  };

  // Handle grant click to show modal - same as grants.tsx
  ...
  // Navigate to previous grant - same as grants.tsx
  ...
  // Navigate to next grant - same as grants.tsx
  ...
  // Optimistic reaction handler - same as grants.tsx
  ...
  const getCategoryDisplayName = (category: GrantCategory) => {
    switch (category) {
      case 'liked':
        return 'Liked Grants';
      case 'disliked':
        return 'Disliked Grants';
      case 'saved':
        return 'Saved Grants';
    }
  };

  const getCategoryIcon = (category: GrantCategory) => {
    switch (category) {
      case 'liked':
        return FiThumbsUp;
      case 'disliked':
  ...
  return (
    ...
    >
      <div className="min-h-screen bg-gray-50">
          ...
          <div className="container-custom py-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-lg sm:text-2xl font-bold text-gray-900">My Grants</h1>
                  <p className="hidden lg:block text-gray-600">View your grant preferences and saved items</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              {(['liked', 'disliked', 'saved'] as GrantCategory[]).map((category) => {
        ...
        <div className="container-custom py-8">
          ...
          {loading && grants.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mt-4">Loading {selectedCategory} grants...</p>
            </div>
          )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-6">
                  <div className="text-sm text-gray-600">
                    Showing <span className="font-semibold text-gray-900">{grants.length}</span> {selectedCategory} grants
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Grants Grid - same as grants.tsx */}
          ...
          {!loading && grants.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No {selectedCategory} grants found</h3>
              <p className="text-gray-600 mb-4">You haven't {selectedCategory} any grants yet.</p>
        ...
        {/* Grant Detail Modal - same as grants.tsx */}
